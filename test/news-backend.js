...
function validateResponse({ response, expectedCount, expectedBtnTitle }) {
    ...
    if (realCount !== expectedCount) {
        throw `bad count: expected=${expectedCount} real=${realCount}`;
    }

    Object.keys(response).forEach(id => {
        const item = response[id];
        if (item.excludeBasedOnOther !== true) {
            throw `missing excludeBasedOnOther: id=${id}`;
        }
        if (item.btnTitle !== expectedBtnTitle) {
            throw `bad btn title: id=${id} expected=${expectedBtnTitle} real=${item.btnTitle}`;
        }
    });
}
